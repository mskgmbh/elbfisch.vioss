 ...
 */
public class ReadRegisters implements Request{
    ...
    public ReadRegisters(Address address){
        if (address.getDataBlock().getValue() != DataBlock.DataBlocks.INTERNALREGISTERS      && 
            address.getDataBlock().getValue() != DataBlock.DataBlocks.PHYSICALINPUTREGISTERS && 
            address.getDataBlock().getValue() != DataBlock.DataBlocks.PHYSICALOUTPUTREGISTERS){
            throw new IllegalArgumentException("register access not allowed for datablock " + address.getDataBlock());
        }
    ...
    protected void writeRequestHeader(Connection conn) throws IOException{
        ...
        conn.getOutputStream().writeShort((short)address.getStartingAddress());                              //address of the first register
        conn.getOutputStream().writeShort((short)address.getSize());                                         //byte count for read access 1-255
    }
    ...
    private short getActualTransactionIdentifier(){
     ...
     public static void main(String[] args){
       ...
       try{
           ...
           ReadRegisters rwreq = new ReadRegisters(new Address(new DataBlock(DataBlock.DataBlocks.PHYSICALCOILS),0,8));
           long startTime;
           long stopTime;
