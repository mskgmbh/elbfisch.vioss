...
import org.jpac.vioss.IOHandlerFactory;
...
public class IoSignalImpl{
    ...
    protected IOHandler getIOHandler() throws InconsistencyException{
        if (getIoHandler() == null){
        	setIoHandler((IOHandler)IOHandlerFactory.getHandlerFor(getUri(), ((Signal)containingSignal).getIoDirection()));
        }
        return getIoHandler();
    }
    ...
    protected void checkIn() throws SignalAccessException {
    	...
    	Value value;
    	RemoteSignalInfo rsi = (RemoteSignalInfo)((IoSignal)containingSignal).getRemoteSignalInfo();
    	synchronized (rsi) {
        	value = rsi.getValue();			
		}
    	if (value.isValid()) {
